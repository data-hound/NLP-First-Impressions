...
def wordVec(word):
    ...
    #Here we are using the symb_dist distionary to get the
    #symbol-distribution stats but in other files we would
    #use the csv file directly
    
    '''
    data=pandas.read_csv(os.getcwd()+"Dictionary/symb_dist.csv")
    '''
    vector = dict()
    vector_key = 'a'
    vector[vector_key] = 0.0
    
    for vector_key in range (ord('a'), ord('z')+1):
        vector[chr(vector_key)] = 0.0
        #print chr(vector_key)
    
    '''print vector.keys()
    print type(vector.keys()[1])
    print type(w[1])
    print type(symb_dist.keys()[0])'''
    
    for i in range(0,l):
        rel_pos= int(round((i/float(l))*100))
        
        vector[w[i]] = symb_dist[w[i]][rel_pos]
...
def prep_dict(doc):
    #doc_dict = list(nltk.word_tokenize(doc))
    doc_dict = set(doc)
    
    vect_dict=dict()
    
    for word in doc_dict:
        vect_dict[word] = wordVec(word)
    ...
    keys = sorted(vect_dict.keys())
    
    
    
    loc_df = pd.DataFrame(vect_dict)
    loc_df = loc_df.transpose()
    loc_df.to_csv(os.path.join(sub_dir,"vec_dict"+".csv"))
    '''with open(os.path.join(sub_dir,"vec_dict"+".csv"), "wb") as outfile:
